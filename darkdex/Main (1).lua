...
local BASE_URL = "https://raw.githubusercontent.com/bladexhub/nexus/refs/heads/main/dardex/"
...
local function fetchModuleSource(name)
	...
	if isfile and readfile and isfile(cachePath) and isfile(metaPath) then
		local okMeta, savedAt = pcall(function() return tonumber(readfile(metaPath)) end)
		if okMeta and savedAt and (os.time() - savedAt) < ModuleCacheTTL then
			local okRead, cached = pcall(readfile, cachePath)
	...
	if ok and result and result ~= "" then
		if writefile and isfolder and makefolder then
			if not isfolder("dex/modules") then pcall(makefolder, "dex/modules") end
			pcall(writefile, cachePath, result)
			pcall(writefile, metaPath, tostring(os.time()))
		end
		return result
	end
...
Main = (function()
	...
	Main.AI = (function()
		...
		local BASE_WIDTH = 1080
		local BASE_HEIGHT = 2400

		local function S(px)
			local viewport = workspace.CurrentCamera.ViewportSize
			local scale = math.min(viewport.X / BASE_WIDTH, viewport.Y / BASE_HEIGHT)
			return math.floor(px * scale + 0.5)
		end

		local ThemeGlassBg = Color3.fromRGB(30,30,38)
		local ThemeStroke = Color3.fromRGB(255,255,255)

		local function getIcon(iconName)
			if not IconLoader or not IconLoader.GetAny then return nil end
			local ok, result = pcall(IconLoader.GetAny, {"lucide","solar","sfsymbols","gravity","swords"}, iconName)
			if ok then return result end
			return nil
		end

		local function applyGlass(frame, radius, strokeTransparency)
			local corner = Instance.new("UICorner")
			corner.CornerRadius = UDim.new(0, radius or S(12))
			corner.Parent = frame
			local stroke = Instance.new("UIStroke")
			stroke.Color = ThemeStroke
			stroke.Transparency = strokeTransparency or 0.9
			stroke.Thickness = 1
			stroke.Parent = frame
		...
		local function bindTouchFeedback(button, restColor, pressColor)
			...
			button.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
					button.BackgroundColor3 = restColor
				end
			end)
		end
		...
		local function resolvePath(path)
			...
			for seg in string.gmatch(trimmed, "([^%.]+)") do
				local ok, found = pcall(function() return cur:FindFirstChild(seg) end)
...
- Your responses are rendered with markdown: **bold**, *italic*, `inline code`, and fenced ```code blocks``` all work. Use them to make output readable but don't over-format.]]
		...
		function AI.Init()
			...
			window:Resize(S(420), S(520))
			AI.Window = window

			local content = window.GuiElems.Content
			content.BackgroundColor3 = ThemeBg

			local bgGradient = Instance.new("UIGradient", content)
			...
			settingsBar.BackgroundColor3 = ThemeGlassBg
			settingsBar.BorderSizePixel = 0
			settingsBar.Position = UDim2.new(0,S(8),0,S(8))
			settingsBar.Size = UDim2.new(1,-S(16),0,S(76))
			settingsBar.Parent = content
			applyGlass(settingsBar, S(16), 0.85)
			...
			modelLabel.TextSize = S(11)
			modelLabel.TextColor3 = ThemeSubtle
			modelLabel.TextXAlignment = Enum.TextXAlignment.Left
			modelLabel.Position = UDim2.new(0,S(10),0,S(4))
			modelLabel.Size = UDim2.new(0,S(60),0,S(12))
			...
			modelButton.TextSize = S(12)
			modelButton.TextColor3 = ThemeText
			modelButton.TextXAlignment = Enum.TextXAlignment.Left
			modelButton.Position = UDim2.new(0,S(10),0,S(16))
			modelButton.Size = UDim2.new(0.5,-S(14),0,S(24))
			modelButton.AutoButtonColor = false
			applyGlass(modelButton, S(8), 0.92)
			bindTouchFeedback(modelButton, ThemeInputBg, Color3.fromRGB(44,44,54))
			local mbPad = Instance.new("UIPadding", modelButton)
			mbPad.PaddingLeft = UDim.new(0,S(30))
			mbPad.PaddingRight = UDim.new(0,S(8))
			AI.ModelDropdown = modelButton

			local modelIcon = Instance.new("ImageLabel", modelButton)
			modelIcon.BackgroundTransparency = 1
			modelIcon.AnchorPoint = Vector2.new(0,0.5)
			modelIcon.Position = UDim2.new(0,S(8),0.5,0)
			modelIcon.Size = UDim2.new(0,S(14),0,S(14))
			modelIcon.Image = getIcon("settings") or ""
			modelIcon.ImageColor3 = ThemeSubtle
			...
			modelButton.MouseButton1Click:Connect(function()
				...
				local nextModel = Models[(idx % #Models) + 1]
				Settings.AI.Model = nextModel.Id
				refreshModelLabel()
			...
			keyLabel.TextSize = S(11)
			keyLabel.TextColor3 = ThemeSubtle
			keyLabel.TextXAlignment = Enum.TextXAlignment.Left
			keyLabel.Position = UDim2.new(0.5,S(4),0,S(4))
			keyLabel.Size = UDim2.new(0.5,-S(14),0,S(12))
			...
			keyBox.Position = UDim2.new(0.5,S(4),0,S(16))
			keyBox.Size = UDim2.new(0.5,-S(14),0,S(24))
			keyBox.Text = Settings.AI.ApiKey or ""
			applyGlass(keyBox, S(8), 0.92)
			local kbPad = Instance.new("UIPadding", keyBox)
			kbPad.PaddingLeft = UDim.new(0,S(30))
			kbPad.PaddingRight = UDim.new(0,S(8))
			AI.ApiKeyBox = keyBox

			local keyIcon = Instance.new("ImageLabel", keyBox)
			keyIcon.BackgroundTransparency = 1
			keyIcon.AnchorPoint = Vector2.new(0,0.5)
			keyIcon.Position = UDim2.new(0,S(8),0.5,0)
			keyIcon.Size = UDim2.new(0,S(14),0,S(14))
			keyIcon.Image = getIcon("lock") or ""
			keyIcon.ImageColor3 = ThemeSubtle
			...
			clearBtn.Text = ""
			clearBtn.Position = UDim2.new(0,S(10),0,S(46))
			clearBtn.Size = UDim2.new(0,S(78),0,S(20))
			applyGlass(clearBtn, S(6), 0.92)
			bindTouchFeedback(clearBtn, ThemeInputBg, Color3.fromRGB(50,32,32))

			local clearIcon = Instance.new("ImageLabel", clearBtn)
			clearIcon.BackgroundTransparency = 1
			clearIcon.AnchorPoint = Vector2.new(0,0.5)
			clearIcon.Position = UDim2.new(0,S(8),0.5,0)
			clearIcon.Size = UDim2.new(0,S(12),0,S(12))
			clearIcon.Image = getIcon("trash-2") or ""
			clearIcon.ImageColor3 = ThemeSubtle

			local clearLabel = Instance.new("TextLabel", clearBtn)
			clearLabel.BackgroundTransparency = 1
			clearLabel.Font = Enum.Font.GothamMedium
			clearLabel.TextSize = S(11)
			clearLabel.TextColor3 = ThemeSubtle
			clearLabel.TextXAlignment = Enum.TextXAlignment.Left
			clearLabel.Position = UDim2.new(0,S(24),0,0)
			clearLabel.Size = UDim2.new(1,-S(28),1,0)
			...
			statusLabel.Position = UDim2.new(0.5,S(4),0,S(46))
			statusLabel.Size = UDim2.new(0.5,-S(14),0,S(20))
			statusLabel.Text = ""
			...
			chatFrame.Position = UDim2.new(0,S(8),0,S(92))
			chatFrame.Size = UDim2.new(1,-S(16),1,-S(160))
			chatFrame.CanvasSize = UDim2.new(0,0,0,0)
			chatFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
			chatFrame.ScrollBarThickness = S(4)
			...
			chatPad.PaddingLeft = UDim.new(0,S(8))
			chatPad.PaddingRight = UDim.new(0,S(8))
			chatPad.PaddingTop = UDim.new(0,S(8))
			chatPad.PaddingBottom = UDim.new(0,S(8))
			AI.ChatMessages = chatFrame

			local inputBar = Instance.new("Frame")
			inputBar.BackgroundColor3 = ThemeGlassBg
			inputBar.BorderSizePixel = 0
			inputBar.Position = UDim2.new(0,S(8),1,-S(56))
			inputBar.Size = UDim2.new(1,-S(16),0,S(48))
			inputBar.Parent = content
			applyGlass(inputBar, S(14), 0.85)

			local inputBox = Instance.new("TextBox", inputBar)
			inputBox.BackgroundColor3 = ThemeInputBg
			inputBox.BorderSizePixel = 0
			inputBox.Font = Enum.Font.Gotham
			inputBox.TextSize = S(14)
			...
			inputBox.Position = UDim2.new(0,S(10),0,S(10))
			inputBox.Size = UDim2.new(1,-S(90),0,S(28))
			inputBox.Text = ""
			applyGlass(inputBox, S(8), 0.9)
			local ibPad = Instance.new("UIPadding", inputBox)
			ibPad.PaddingLeft = UDim.new(0,S(10))
			ibPad.PaddingRight = UDim.new(0,S(10))
			...
			sendBtn.Position = UDim2.new(1,-S(74),0,S(10))
			sendBtn.Size = UDim2.new(0,S(64),0,S(28))
			applyGlass(sendBtn, S(8), 0.75)
			bindTouchFeedback(sendBtn, ThemeAccent, Color3.fromRGB(8,105,204))

			local sendIcon = Instance.new("ImageLabel", sendBtn)
			sendIcon.BackgroundTransparency = 1
			sendIcon.AnchorPoint = Vector2.new(0.5,0.5)
			sendIcon.Position = UDim2.new(0.5,0,0.5,0)
			sendIcon.Size = UDim2.new(0,S(16),0,S(16))
			sendIcon.Image = getIcon("send") or ""
